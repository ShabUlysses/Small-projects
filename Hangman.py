...
def get_word(lst):
    return choice(word_list).capitalize()
...
def play(word):
    print('Давай сыграем с тобой в игру. Я загадал слово, а ты попробуй его отгадать.')
    answer = get_word(word_list)
    tries = 6
    tried_letters = []
    
    
    display_word = ['_' for _ in range(len(answer))]
    word_storage = [letter for letter in answer]
    print(*display_word)
    print()
    print()
    ...
    while tries > 0:
        ...
        print(f'У тебя есть {tries} попыт{ending}. \n')
        print('Ты уже пробовал следующие буквы: ', *tried_letters)
        
        
        trigger = True
        while trigger:
            guess = input('Начинай отгадывать. Ввести можно только одну букву русского алфавита \n')
            if len(guess) != 1 or ord(guess.lower()) > 1104 or ord(guess.lower()) < 1071:
        ...
        if guess.lower() in answer.lower():
            for index, char in enumerate(word_storage):
            ...
            tries -= 1
            if tries == 0:
                print(f'Ты проиграл, загаданным словом было {answer}')
                print(display_hangman(tries))
                break
        tried_letters.append(guess.lower())
        
        
        print(*display_word)
        
        print('Рисую виселицу')
        print(display_hangman(tries))
        
        guess_the_word = input('Может, готов отгадать слово целиком? Если ошибёшься, то сниму с тебя попытку. (Да/Нет) \n ')
        if guess_the_word.lower() == 'да':
            whole_word = input('Введи слово \n')
            if whole_word.lower() == answer.lower():
                print('Ура! Ты угадал!')
                break
            else:
                print('Ноуп.')
                ...
                if tries == 0:
                    ...
                    print(display_hangman(tries))
                    break
        elif guess_the_word.lower() == 'нет':
            continue
        else:
            print('Я тебя не понял или ты ввёл ответ с ошибкой. Будем считать, что ты отказался отгадать слово. \n')
...
while True:
    ...
    if answer.lower() == 'да':
        ...
        time.sleep(1)
        print('Ладно, увидимся в следующий раз.')
        break
